<template>
  <Transition name="modal" appear>
    <div v-if="isVisible && !isMinimized" class="fixed inset-0 z-[85] flex items-center justify-center p-4">
      <div class="absolute inset-0" @click="$emit('close')"></div>

      <div class="relative w-full max-w-xl bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-600 rounded-xl p-5 shadow-2xl overflow-hidden">
        <!-- Header -->
        ...
        <p class="text-gray-700 mb-3">Pilih hadiah karena menjawab <span class="font-semibold">benar</span>:</p>
        ...
        <div class="flex flex-col gap-3">
          ...
          <div v-if="showShoot" class="mt-1 bg-white rounded-lg p-3 border border-amber-300 shadow-sm">
...
const shootableOpponents = computed(() => props.players.filter(p => p.id !== props.currentPlayerId && !p.finished))
...
.modal-enter-from { opacity: 0; transform: scale(0.9) translateY(-6px); }
.modal-leave-to { opacity: 0; transform: scale(0.95) translateY(6px); }
</style>
